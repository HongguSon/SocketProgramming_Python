...
class FileTransfer:
    def __init__(self) -> None:
        ...
        self.udp_ack_num = 0 # 수정사항
        self.udp_last_ack_num = 0
        self.file_packet_start = 0 # 수정사항
        self.file_name = None
    ...
    def tcp_file_send(self, filename: str, tcp_send_func: Callable)-> None:
        ...
        while True:
            ...
            if packet_tuple[0]:
                ...
                print("패킷전송완료", len(packet_tuple[1]))
            else:
                break
    ...
    def udp_file_data(self) -> Tuple[bool, bytes]:
        # udp sender가 전송할 file data를 얻는다
        # 결과값: file data
    ...
    def udp_send_with_record(self, packet_type: bytes, data: bytes, udp_send_func: Callable) -> None:
        print("UDP_SEND_WITH_RECORD 함수 호출,", self.udp_ack_num, self.udp_last_ack_num) # 디버깅용
    ...
    def udp_file_send(self, filename: str, udp_send_func: Callable) -> None:
        ...
        while data_ready:
            print("전송한 패킷의 양은?", len(self.udp_send_packet), len(data))
            if len(self.udp_send_packet) < UDP_WINDOW_SIZE: #window의 크기보다 전송한 패킷의 양의 적은 경우
                ...
                print("윈도우사이즈가 넘어서 pipeline 써야함")
        ...
        # self.udp_pipeline(udp_send_func) 이거 해야함
    ...
    def udp_file_receive(self, packet: bytes, udp_send_func: Callable) -> int:
        ...
        print("udp_file_receive 함수 호출", packet_type, ack_num, self.udp_last_ack_num, self.udp_ack_num)
        ...
        if packet_type == PACKET_TYPE_FILE_START:  # file transfer start
            ...
            print("패킷 스타트 넘버와 ACK 넘버 비교", self.file_packet_start, ack_num, self.udp_last_ack_num)
            return 0

        elif packet_type == PACKET_TYPE_FILE_DATA:  # file transfer
            print("데이터 수신시작, 패킷타입 데이터", ack_num, self.file_packet_start)
            ...
            while self.udp_recv_packet[self.file_packet_start] != bytes(PACKET_SIZE):
                print("수신 후 저장 시작!")
                self.file_pointer.write(data)
                self.udp_recv_flag[self.file_packet_start] = True
            ...
            print("ACK가 언제 들어오는지 확인", self.udp_ack_num, ack_num)
            if self.udp_ack_num == ack_num:
                self.udp_ack_windows[ack_num] == True
                self.udp_ack_num += 1
                del self.udp_send_packet[ack_num]
                
            # for check_num in range(self.udp_ack_num, ack_num+1):
            #     if self.udp_ack_windows
    ...
    def udp_pipeline(self, udp_send_func: Callable) -> None:
        ...
        def udp_gbn():
            # if self.udp_time_out():
            #     udp_send_func(self.udp_send_packet[self.udp_ack_num])
            # else:
            #     pass
            udp_send_func(self.udp_send_packet[self.udp_ack_num])
        udp_gbn()
