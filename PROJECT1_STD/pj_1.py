...
class NetworkSocket:
    ...
    @staticmethod
    def tcp_server_connect(sock: socket.socket) -> Tuple[socket.socket, any]:
        # Server_socket을 통해 client와의 connection 생성 
        # 생성된 connection socket(conn)과 client의 address(tcp_client_addr) 반환
        max_num = 5
        sock.listen(max_num)
        conn_sock, adrr = sock.accept()
        return (conn_sock, adrr)
    ...
    def tcp_client_socket(host: str, port: int) -> socket.socket:
        ...
        tcp_client_sock.connect((host, port))
    ...
    def udp_server_connect(udp_server_socket: socket.socket):
        ...
        while True:
            _, udp_client_addr = udp_server_socket.recvfrom(4096)  
            return udp_client_addr
    ...
    def udp_client_socket(host: str, port: int) -> socket.socket:
        ...
        udp_client_sock.sendto(b'', (host, port))
        return udp_client_sock
    ...
    def tcp_recv(self) -> bytes:
        ...
        buff_size = 4096
        data = self.tcp_socket.recv(buff_size)
        return data

    def udp_recv(self) -> bytes:
        # UDP socket(udp_socket)으로 들어오는 packet의 data 반환 
        buff_size = 4096
        data, _ = self.udp_socket.recvfrom(buff_size)
        return data
